...
When I say "draft" / "draft a comment" / "draft this" / "draft a reply" / "draft for <X>", show the body inline in chat and stop. No `AskUserQuestion` with a "Post it" option. No `gh` / `glab` invocation. No examples-log append. The post phase begins only when I separately say "post" / "post it" / "send it" / equivalent. Bundling them (drafting + post-option AskUserQuestion in one move) cuts me out of the gate I asked for.
