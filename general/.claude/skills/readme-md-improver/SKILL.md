...
- **Cut internal mechanics that are transparent to the user.** Lines describing *how* the tool finds, resolves, or wires something internally ("the plugin resolves the in-tree binary directly", "shells out to install.sh with PREFIX pointing at...", "the resolver prefers X and falls back to Y", "uses go install if available, otherwise downloads a prebuilt release") explain implementation the user never sees and can't act on. Keep only the parts that have a user-visible consequence: where the artifact lands (so they can delete it), whether their `$PATH` is touched (so they can predict shell pollution), what to do if it fails. Drop the rest. Smell test: if the only thing the line tells the reader is *how the code works internally*, it's noise — the code is the doc for code.
