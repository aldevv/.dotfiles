...
description: "Improve a single lazy-load trigger entry in a CLAUDE.md file (a `- [<path>](<href>). **Read when** any of:` block under the Lazy load section). Generates ~20 realistic should-fire / should-NOT-fire eval queries, runs them via the bundled harness (each query spawns `claude -p` against a sandboxed CLAUDE.md containing only the trigger, detects fire by watching stream events for a `Read` of the trigger target), iterates up to 5 rounds with Claude proposing minimal edits per misfire, picks the winner by held-out test score, and applies it back to the file. Triggers on '/trigger-improver', 'improve this trigger', 'audit the trigger for X', 'tighten the trigger for X', 'optimize the trigger for X', 'fix the trigger for X', or any explicit request to optimize/audit a `**Read when**` block. Target resolution: explicit path wins; 'global' or 'global claude' targets `~/.dotfiles/general/CLAUDE.md`; otherwise `./CLAUDE.local.md`, then `./CLAUDE.md`, then global. Do NOT trigger for restructuring a CLAUDE.md (use `claude-md-simplify`), adding a new rule (use `claude-md-save`), or auditing a SKILL.md description (use `skill-creator:skill-creator`)."
---
...
1. **User named a target.** Match by the linked path (`~/.claude/lazy/code/quality.md`), the file basename (`quality.md`), or a section heading they reference. Confirm the match in one sentence. When the CLAUDE.md has two entries whose basenames collide (e.g. multiple `quality.md` files at different paths), pass the full path to `--target`, not the basename: the harness's substring fallback would pick the first match and silently optimize the wrong entry.
