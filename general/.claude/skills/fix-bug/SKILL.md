...
When the cwd is inside a git repo, write the operator-only artifacts under `<repo>/.inreview/<DATE>/fix-bug/<key>/` so the dispatch-resume contract (auto-new-day's per-date archive) picks them up. Otherwise, fall back to `/tmp` (operator outside a repo, no archive needed).

```
<repo>/.inreview/<DATE>/fix-bug/<key>/
