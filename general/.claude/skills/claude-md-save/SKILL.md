---
name: claude-md-save
description: "Save a rule, preference, or directive into the right CLAUDE file for the current project. Triggers when the user says any of: 'save in my claude', 'save this in claude', 'add this to my claude', 'add to my CLAUDE.md', 'save this rule', 'put this in my claude', 'remember this in my CLAUDE.md', 'save this to my claude', or any phrasing that expresses intent to persist a preference or rule into a CLAUDE.md / CLAUDE.local.md / .claude/lazy/*.md file. Do NOT trigger for reading or auditing CLAUDE files (use `claude-md-simplify` for restructuring, `claude-md-management:revise-claude-md` for broader rewrites), or for the global `~/.claude/CLAUDE.md` unless the user says 'global claude'. CRITICAL: 'save in my claude' ALWAYS means CLAUDE files only. It NEVER means auto-memory, memory files, or anything under a memory/ directory. The user has auto-memory disabled and does not want it."
---
