...
Cross-machine entries (PR/MR watch flow `pr-watch.sh`, `notify.sh`, statusline, generic permissions, `autoMemoryEnabled`, etc.) live in `$HOME/.dotfiles/general/.claude/my-settings.json`. That file is the canonical "travels via dotfiles" copy. Every `sync-dotfiles` run applies `my-settings.json` as a managed overlay onto `$HOME/.claude/settings.json`: added/changed entries flow in, removed entries are stripped when the live value still matches what was previously applied. Use `$HOME/...` (not `/home/<user>/...` or `/Users/<user>/...`) for command paths in `my-settings.json` so entries port to every machine.

**CRITICAL: edit `my-settings.json` for anything cross-machine, not `settings.json` alone.** If you remove a hook / permission / env var / statusLine entry only from `~/.claude/settings.json` but leave it in `my-settings.json`, the next `sync-dotfiles` run will overlay it right back. Same for adds and changes. The rule of thumb: if the entry is a candidate to travel with the dotfiles (it references portable `$HOME/...` paths, applies on every machine, isn't purely local), the edit lives in `my-settings.json`; propagate to `settings.json` in the same session so the change takes effect immediately without waiting for a sync. Local-only entries (machine-specific paths, work-only tooling, plugin internals, auto-written fields like `feedbackSurveyState`) stay in `settings.json`.
