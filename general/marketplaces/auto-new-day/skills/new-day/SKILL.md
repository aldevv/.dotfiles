...
- **Step 2b (empty in-progress).** If zero in-progress tickets survive:
  ...
  - **`discovery.todo_fallback: "top-one"`.** Promote exactly ONE `Todo` ticket into `In Progress`, then classify + dispatch it like any in-progress item. Steps: (1) `list_issues` with `team` + `assignee=me.id` + status `Todo` (the `unstarted` status), paginate to `hasNextPage == false`, and re-verify `assignee.id == me.id` per result (drop mismatches); (2) if none, dispatch nothing and report; (2b) **drop any `Todo` blocked by an unmerged ticket.** For each candidate, `get_issue` with `includeRelations: true` and read `relations.blockedBy`; a candidate is disqualified if ANY `blockedBy` ticket is not yet resolved (its status is not a `completed`/Done type AND, when it has a linked PR, that PR is not merged). An empty `blockedBy` passes. If every candidate is blocked, dispatch nothing and report (do NOT promote a blocked ticket just because it is highest priority); (3) else, among the surviving unblocked candidates, pick the single highest-priority ticket, where Linear `priority.value` orders Urgent(1) > High(2) > Medium(3) > Low(4) > No-priority(0) (so rank by `value == 0 ? 5 : value`, ascending), tie-break by newest `updatedAt`; (4) `save_issue` to set its status to the team's `In Progress` status id, then **re-read the issue and confirm the status actually changed** (a `save_issue` success line is not proof, per the connector-write caveat); (5) compute its `workflow` from labels and dispatch through the normal `inprogress` path (`bucket_skills.inprogress` label routing) exactly as if it had been discovered in progress. Never promote more than one; never touch `Backlog`/`Triage`; never set `assignee`. Record the promotion in the Step 6 report with a `⚠️` (the sweep changed tracker state) and note the ticket + why (In-Progress was empty).
...
1. **Build the window list.** For each item, run `${SCRIPTS}/triage.sh` (dispatch / resume / skip / discard). It checks the marker (skip if fresh, unless `--force`), runs `marker.sh is-abandoned` for crash recovery, and searches prior dates for a resume manifest. Drop discarded items with a one-line note. **When an OWN-work item (`inreview`/`inprogress`) triages `skip` or `resume` because a PRIOR SWEEP already worked it, do NOT just note "skipped" in chat.** That prior work is the whole point of the sweep and needs the operator's eyes, so **open a follow-up window** for it (the recommended actions belong in a dispatched window, not only the chat report):
   - Resolve its CURRENT state: local commits on the branch vs `origin` (`git -C <repo> log --oneline @{upstream}..`, or vs the base branch when unpushed), whether a PR now exists and its review/CI/merge state, and the ticket's Linear status.
   - Write a SHORT body file (`$DATE_DIR/dispatch/followup-<ITEM>.body.md`) — a compact **markdown table** of the key facts, NOT prose and NEVER a JSON blob. One header line (`<TICKET> · <repo> · <short title>`), then a small table with rows like `Status`, `PR`, `Branch`, `State`, `Prior sweep` (pointer to `dates/<D>-report.md`), then a single `Recommended:` line with 2-3 numbered next steps (push + open PR; rebase; address a reviewer; get an approver + merge; nothing, already merged). Keep the whole file to ~10 lines: the pager is a glance, the detail lives in the repo/PR. If a stray JSON manifest got appended to this file by a prior worker, overwrite it clean.
   - Open the window with `${SCRIPTS}/rtm-window.sh --session AUTO-followup --window <ITEM>-<repo> --repo-dir <repo> --branch <branch> --status followup --body-file <body> [--review-diff <base>]`. The `followup` shape gives a **claude session on the branch** in the left pane (seeded with the body as context, told to summarize then WAIT, no push/PR/merge until the operator says so) so the operator can just say "push" / "open the PR" / "merge". **When there are local changes to review** (unpushed commits or a dirty tree), pass `--review-diff <base>` (e.g. `origin/<default-branch>` or the merge-base): the right side then shows the ticket-summary/recommended-actions pager on top AND a **Hunk** pane (`hunk diff <base> --exclude-untracked`) below it, so the operator gets the summary and the diff next to the claude pane. When there's nothing local to review (e.g. the work is a PR already on origin), omit `--review-diff` and the right pane is just the pager. Dedupes/replaces by window name on re-run.
   - It is NOT re-dispatched to a worker child (that would duplicate the local work) unless `--force`. In the Step 6 report, add just a one-line pointer per item ("already-handled → see `AUTO-followup:<window>`"). This follow-up window is the default for own-work skip/resume, not a silent drop.
   - **Repeat-groundhog check (self-resolve, don't re-ask).** Before writing today's body file, check the most recent prior `followup-<ITEM>.body.md` for this item. If today's recommendation would be unchanged from that prior one AND the recommendation itself is an abandon/superseded call — e.g. a parent/umbrella ticket whose real work now lives entirely in separate child tickets that already have their own PRs, or a prior body already said "no new work needed here" — do not open a third (or later) window just to re-state it. Instead call `${SCRIPTS}/mark_done.sh --bucket <bucket> --key <ITEM> --repo-dir <repo> --forever --outcome skip --reason "<why, name the superseding tickets/PRs>"` yourself, skip the window, and add one line to the Step 6 report instead (`<ITEM>: superseded by <children>, marked done forever, no window opened`). This is local bookkeeping, not a push/PR/merge, so it does not need the operator's go-ahead. Reserve the followup window for recommendations that still need the operator's judgment call (push readiness, rebase timing, which PR to merge, addressing a reviewer) — those keep getting a window every time state changes; only "there is nothing left to ship here" self-resolves.
...
3. **Append to create.md.** One `## AUTO-<bucket>` section, one `### Window: <name>` per item, each with a `#### bootstrap` block (`${SCRIPTS}/bootstrap-window.sh --profile <own-work|review> --date-dir <d> --repo-dir <r> [--ticket <t> | --window <w>] --final-cwd <working_root>`) and a `#### claude invocation` line (`claude --dangerously-skip-permissions "<bucket_skills[bucket]> [--no-subagents] <arg>"`). The `--dangerously-skip-permissions` flag is mandatory: dispatched children run unattended and must not stall on a permission or folder-trust prompt (`tmux-dispatch.sh` documents the same, and `launch-auto-new-day.sh` / `rtm-window.sh` already pass it). Keep this format stable, it is what Step 0.5 / date-replay parse.

   **`<name>` for `inreview`/`inprogress` is `<ITEM>-<repo>`, never a bare ticket ID.** Same convention the followup window already uses (item 1 above). `<repo>` is the repo's own name (e.g. `baton-databricks`), NOT the checkout directory's basename — under `USE_WORKTREE=true` (Step 6.7) the checkout lives at `~/worktrees/<repo>/<branch>`, whose basename is the branch, not the repo. Derive it from the git remote instead: `basename -s .git "$(git -C <r> remote get-url origin)"`. A bare ticket ID leaves every tmux window and log file for a multi-connector sweep looking identical until you attach; the repo suffix makes the window list scannable on its own (`tmux list-windows`, the `create.md` outline, `$DATE_DIR/logs/*.log` filenames all read directly off this name). `inreview-others` windows already carry the repo name (e.g. `baton-foo-42`); no change needed there.
4. **Spawn (real run only).** `${SCRIPTS}/tmux-dispatch.sh <session> <win> <cwd> <bootstrap-cmd> <slash-invocation> --log <path>`, where `<win>` is the same `<name>` computed in item 3 (`<ITEM>-<repo>` for `inreview`/`inprogress`). It creates the session on the first window and dedupes by window name. Use `${SCRIPTS}/refresh-decision.sh` to decide dispatch / skip / poke / replace for an already-running window. (Each dispatched skill opens its own PR/ticket context pager at close via the shared `dispatch-closing.md` contract, so the engine does NOT add a pager pane here. `followup` and `ready-to-merge` windows get their pager from `rtm-window.sh`.)
